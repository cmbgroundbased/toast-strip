...
OmpManager::OmpManager() : mem_size_(), mem_() {
    ...
    if (dev_null_ == NULL) {
        o.str("");
        o << "OmpManager:  on construction, failed to allocate dummy dev pointer";
        log.error(o.str().c_str());
        throw std::runtime_error(o.str().c_str());
    }
    failed = omp_target_associate_ptr(
        null, dev_null_, sizeof(int64_t), 0, target_dev_
    );
    if (failed != 0) {
        o.str("");
        o << "OmpManager:  on construction, failed to associate dev null pointer";
        log.error(o.str().c_str());
        throw std::runtime_error(o.str().c_str());
    }
    #else // ifdef HAVE_OPENMP_TARGET
...
OmpManager::~OmpManager() {
    clear();
    #ifdef HAVE_OPENMP_TARGET
    int failed = omp_target_disassociate_ptr(null, dev_null_);
    if (failed != 0) {
        o.str("");
